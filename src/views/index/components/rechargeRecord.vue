<template>
  <div class="box">
    <div class="top-box">
      <div class="date-container" @click="checkTime">
        <div v-if="date1 && date==''">{{date1}}月</div>
        <div v-if="date">{{date}}月</div>
    ...
    <List v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad" class="list">
      <div class="item-box" v-for="(item,index) in list" :key="index" @click="toDetail(item)">
        <Cell>
          ...
          <template #default>
            ...
            <span class="cell-value">{{ item.payStatus !=1 ?usedList2[item.payStatus]:usedList3[item.isUsed] }}</span>
          </template>
        </Cell>
...
export default {
  ...
  data() {
    return {
      usedList2: ['未付款', '充值中', '充值失败', '已退款', '退款中', '部分退款'],
      usedList3: ['未到账', '已到账', '部分到账'],
      // date: moment().startOf('month').format('YYYY-MM'),
      date: '',
      date1: moment().startOf('month').format('YYYY-MM'),
