<template>
  <div class="telephoneExpenses">
    ...
    <div class="centre">
      <!-- <NoticeBar scrollable speed="100" background="#fff9f9" color="#fb584d"> {{productList.notes.replace(/<\/?.+?>/g,'')}}
        <template #left-icon>
    ...
    <div class="centre" v-if="productAttributeConfigVOList && productAttributeConfigVOList.length>0">
      <div class="centre-title">
        <span>{{ productList1.propaganda }}</span> <i>（10分钟-3小时内到账）</i>
      </div>
      <div class="centre-item">
        <div @click="goPrice(item)" v-for="item in productAttributeConfigVOList" :key="item.id" class="centre-item-Num">
          <span>{{ item.originalPrice
            }}<em style="font-style: normal;font-size: 13px;">元</em></span>
          <span> <i>售价</i> ￥{{ item.amountPrice }}</span>
        </div>
    ...
    <van-dialog v-model="showDialog" class="box3" :showConfirmButton="false">
      ...
      <RadioGroup v-model="radio" style="font-size:13px" checked-color="#F14F44" icon-size="15px">
        <CellGroup :border="false">
          <!-- v-if="(Number(form.checkCashnuber) < Number(integral)) && product.paymentMode && product.paymentMode.includes('0')" -->
          ...
          <Cell clickable @click="radio = '2'" :border="false" class="wechat-pay-cell" v-if="items.supplierType==3">
            <div class="wechat-pay">微信支付</div>
            <template #right-icon>
              <Radio name="2" style="margin:10px 0" class="wechat-pay-outer"></Radio>
            </template>
          </Cell>
          <Cell clickable @click="radio = '3'" :border="false" v-if="items.supplierType==3 && Number(items.amountPrice) > Number(integral)" class="combined-payment-cell">
...
export default {
  ...
  watch: {
    ...
    active: {
      handler(val) {
        ...
        // console.log(val);
        if (val == 0) {
          console.log('22312312', this.ChinaMobile);

          this.getData(this.ChinaMobile)
          this.getData1(this.ChinaMobile1)
        } else if (val == 1) {
          console.log('222');
          this.getData(this.ChinaUnion)
          this.getData1(this.ChinaUnion1)
        } else if (val == 2) {
          console.log('222');

          this.getData(this.ChinaTelcom)
          this.getData1(this.ChinaTelcom1)
        }
      },
      immediate: true,
      deep: true
  ...
  computed: {
    ...
    productAttributeConfigVOList() {
      if (this.productList1 && this.productList1.productAttributeConfigVOList) {
        return this.productList1.productAttributeConfigVOList.filter(item => {
          return Number(item.amountPrice) < Number(this.integral)
        })
      }
    }
  },
  async created() {
    ...
    //测试分享
    // await this.wxH5Share.getConfig('话费充值只要94折', '电信、联通、移动三大运营商都在！', `https://jhbtestapp.gongxingtech.com/#/telephoneExpenses?productId=${this.productId}&shareId=${userInfo.openid}&one=24,25,26&Mmid=69`)    // this.wxH5Share.getConfig()
  },
  mounted() {
    this.$nextTick(() => {
      setTimeout(() => {
  ...
  methods: {
    ...
    getProductConfigInfo(productIdLists) {
      if (productIdLists && productIdLists[0]) {
        this.getID(productIdLists[0])
      }
      if (productIdLists && productIdLists[1]) {
        this.getID(productIdLists[1])
      }
      if (productIdLists && productIdLists[2]) {
        this.getID(productIdLists[2])
      }
      if (productIdLists && productIdLists[3]) {
        this.getID(productIdLists[3])
      }
      if (productIdLists && productIdLists[4]) {
        this.getID(productIdLists[4])
      }
      if (productIdLists && productIdLists[5]) {
        this.getID(productIdLists[5])
      }
    },
    getID(id) {
      getProductConfigInfo({ id }).then(v => {
        ...
        if (v.data.name === '电信直充') {
          ...
          this.ChinaTelcom1 = v.data.id //电信直冲
          storageSet('ChinaTelcomId1', v.data.id)
          // this.getData1(this.ChinaTelcom1)
        }
        if (v.data.name === '移动慢充') {
        ...
        if (v.data.name === '移动直充') {
          this.ChinaMobile1 = v.data.id //移动直冲
          storageSet('ChinaMobileId1', v.data.id)
          this.getData1(this.ChinaMobile1)
        }
        if (v.data.name === '联通慢充') {
        ...
        if (v.data.name === '联通直充') {
          storageSet('ChinaUnionId1', v.data.id)
          this.ChinaUnion1 = v.data.id
          // this.getData(this.ChinaUnion1)
        }
      })
    },
    closeDialog() {
    ...
    goPrice(item) {
      ...
      if (item.productId == this.ChinaMobile || item.productId == this.ChinaMobile1) {
        if (!isChinaMobile.test(this.tel)) {
          Toast('非移动号码')
          return
        } else {
      ...
      } else if (item.productId == this.ChinaUnion || item.productId == this.ChinaUnion1) {
      ...
      } else if (item.productId == this.ChinaTelcom || item.productId == this.ChinaTelcom1) {
