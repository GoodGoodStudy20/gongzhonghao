<template>
  <div class="telephoneExpenses">
    ...
    <div class="payment-account">
      <Cell is-link :border="false" class="cell">
        <template #title>
          <div class="left">
            <img src="@/assets/img/power.png" alt="">
            <div class="left-container">
              <div class="left-top">户号：1234567****34567</div>
              <div class="name-adress"><span>*利</span> <span>广东深圳</span></div>
            </div>
          </div>
        </template>
      </Cell>
    ...
    <div class="phone">
      <Field :border="false" v-model="tel2" type="tel" maxlength="11" label="缴费户号：" placeholder="请输入缴费户号" />
    </div>
    <div class="phone" v-if="active==1">
      <Field :border="false" v-model="tel" type="tel" label="用户信息：" placeholder="如身份证/ 营业执照/银行卡后六位" />
    </div>
    <div class="recharge-btn" :disabled="!isRecharge" @click="toRecharge" :class="{'active':isRecharge}">
      去充值
    </div>
    <!-- <div class="centre">
...
export default {
  ...
  methods: {
    toRecharge() {
      this.$router.push({ path: '/powerRechargeDetail' })
    },
    confirm(value) {
