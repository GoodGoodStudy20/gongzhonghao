<template>
  <div class="box">
    <AddressEdit :area-list="areaList" :show-delete="false" :address-info="adressData" show-set-default show-search-result :search-result="searchResult" :area-columns-placeholder="['请选择', '请选择', '请选择']" @save="onSave" />
  </div>
</template>
...
import { getProductConfigInfo, countVouchersInterests, addProductRecharge, addShoppingAddress, getShoppingAddressPage, updateShoppingAddress } from '@/api/user'
import { AddressEdit, Button, Field, Dialog, Toast, RadioGroup, Radio, Cell, CellGroup, Popup } from 'vant'
import { Debounce, storageSet, storageGet, NumberSub } from '@/utils/utils'
export default {
  name: 'addAdress',
  components: { AddressEdit, Button, Field, RadioGroup, Radio, Cell, CellGroup, Popup },
  data() {
    return {
      integral: 0,
      bindPhone: '',
  ...
  created() {
    ...
    if (this.$route.query.id) {
      this.adressId = this.$route.query.id
  ...
  methods: {
    getShoppingAddressPage() {
      ...
      getShoppingAddressPage(param).then(res => {
        ...
        list.forEach(item => {
          if (item.id == this.adressId) {
            this.adressData = item
          }
        })
        this.adressData.tel = this.adressData.mobile
        this.adressData.province = this.adressData.province
        this.adressData.city = this.adressData.city
        this.adressData.county = this.adressData.district
        this.adressData.addressDetail = this.adressData.address
        this.adressData.isDefault = this.adressData.isDefault == 1 ? true : false
        console.log(this.adressData);
        console.log(this.adressData.county);
        for (let i1 in this.areaList.city_list) {
          if (this.areaList.city_list[i1] == this.adressData.city) {
            console.log(i1);
            let a = i1.slice(0, 2)
            for (let i in this.areaList.county_list) {
              if (this.areaList.county_list[i] == this.adressData.county) {
                let b = i.slice(0, 2)
                if (a == b) {
                  this.areaCode = i
                }
        ...
        this.adressData.areaCode = this.areaCode
      })
    },
    onSave(val) {
      console.log('保存地址', val);
      let param
      if (val) {
        param = {
          userId: this.userInfo.id,
      ...
      if (this.adressId) {
        param.id = this.adressId
        updateShoppingAddress(param).then(res => {
          if (res.code !== 200) return
          Toast('修改地址成功')
          this.$router.go(-1)
        })
      } else {
        addShoppingAddress(param).then(res => {
          if (res.code !== 200) return
          Toast('添加地址成功')
          this.$router.go(-1)
        })
