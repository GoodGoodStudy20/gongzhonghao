  ...
  <div class="box">
    <div class="qrcode-img">
      <div class="contennier">
        恭喜您！充值到账成功 <span>{{amount || price}}元</span>
      </div>
      <div class="my-pay-show">在“我的-全部订单”中查看使用</div>
      <div class="gd-code" v-if="city=='gd'">
        <img src="@/assets/img/gotoOil.png" alt="">
        <wx-open-launch-weapp id="launch-btn" :username="ghId[city]" path="">
      ...
      <div style="font-size:14px;text-align:center;margin-top:10px">{{topTip}}</div>
    </div>
  ...
  computed: {
    codeList() {
      let list = [
        ...
        '因系统清算，每月末最后一天和次月初第一天暂停使用电子加油券；',
        '加油券充值为虚拟充值，支付成功后不支持退款充值时请仔细核对手机号码；',
    ...
    topTip() {
      return this.city == 'hn' ? "前往中石化小程序 “易捷加油”" : "前往中石化小程序 “大家来加油”使用"
    }
  },
  methods: {
